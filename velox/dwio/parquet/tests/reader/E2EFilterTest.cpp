...
TEST_F(E2EFilterTest, writeDecimalAsInteger) {
  ...
  EXPECT_EQ(c0->parquetType_.value(), velox::parquet::thrift::Type::INT32);
  auto c1 = std::dynamic_pointer_cast<const ParquetTypeWithId>(types[1]);
  EXPECT_EQ(c1->parquetType_.value(), velox::parquet::thrift::Type::INT64);
  auto c2 = std::dynamic_pointer_cast<const ParquetTypeWithId>(types[2]);
  EXPECT_EQ(c2->parquetType_.value(), velox::parquet::thrift::Type::FIXED_LEN_BYTE_ARRAY);
}
