...
uint32_t HiveDataSink::ensureWriter(const WriterId& id) {
...
uint32_t HiveDataSink::appendWriter(const WriterId& id) {
  VELOX_USER_CHECK_LE(
  ...
  addThreadLocalRuntimeStat(
      fmt::format(
          "{}WriterCount",
          dwio::common::FileFormatName::toName(insertTableHandle_->storageFormat())),
      RuntimeCounter(1));
