...
TEST_F(IcebergInsertTest, rejectNestedUnknown) {
  const auto outputDirectory = TempDirectoryPath::create();
  // UNKNOWN nested inside ARRAY, MAP value, or an all-UNKNOWN ROW is rejected.
  VELOX_ASSERT_THROW(
      createDataSink(
          ROW({"id", "arr"}, {BIGINT(), ARRAY(UNKNOWN())}),
          outputDirectory->getPath()),
      "Writing to a column with UNKNOWN type nested inside a complex type is not supported. Column: arr");
  VELOX_ASSERT_THROW(
      createDataSink(
          ROW({"id", "m"}, {BIGINT(), MAP(VARCHAR(), UNKNOWN())}),
          outputDirectory->getPath()),
      "Writing to a column with UNKNOWN type nested inside a complex type is not supported. Column: m");
  VELOX_ASSERT_THROW(
      createDataSink(
          ROW({"id", "r"}, {BIGINT(), ROW({"x"}, {UNKNOWN()})}),
          outputDirectory->getPath()),
      "Writing to a column with UNKNOWN type nested inside a complex type is not supported. Column: r");
